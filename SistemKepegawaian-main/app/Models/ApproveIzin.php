...
class ApproveIzin extends Model
{
    use HasFactory;
    use SoftDeletes;
    ...
    public static function do_update($id, Request $request)
    {
        ...
        try {
            ...
            if ($izin == null) return ResponseHelper::response_error("Proses Approve Gagal", "Data tidak ditemukan");
            ...
            // Setelah input / approve data lakukan commit agar perubahan tersimpan di database
            DB::commit();
            return ResponseHelper::response_success("Proses Aprove Berhasil", "Data telah disimpan");
        } catch (Exception $e) {
            // Jika terjadi kesalahan waktu input, lakukan aksi rollback sehingga data tidak tersimpan
            DB::rollBack();
            return ResponseHelper::response_error("Proses Approve Gagal", "Proses input data gagal!");
        }
    }
